...
def load_css():
    st.markdown("""
        ...
        h1 {color: #58a6ff !important; text-align: center; margin-bottom: 2rem;}
        .stButton > button {width: 100%; background: linear-gradient(45deg, #58a6ff, #238636) !important; color: white !important;}
        .streamlit-expanderHeader {background-color: #161b22 !important; color: #c9d1d9 !important;}
...
def main():
    try:
        ...
        if st.button("Scan for Patterns", key="scan_button"):
                ...
                with st.expander(f"{company_name} ({ticker})"):
                    try:
                        ...
                        if img:
                            ...
                            st.error(f"Could not generate chart for {ticker}")
                    except Exception as e:
